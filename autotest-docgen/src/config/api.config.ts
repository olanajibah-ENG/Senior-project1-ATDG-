...
export const API_ENDPOINTS = {
  ...
  // 7. كشف التناقض (Conflict Detection) — /api/analysis/
  conflict: {
    /** POST /api/analysis/upload-folder/
     *  body: FormData { file, upm_project_id, project_name }
     *  يُستخدم لرفع كل نسخة (ZIP) قبل بدء التحليل
     */
    uploadFolder: () => buildUrl(API_CONFIG.PREFIX.AI, 'upload-folder/'),

    /** POST /api/analysis/detect-conflict/
     *  body: { analysis_type, project_id, file_id, version_a_id, version_b_id }
     *  يبدأ عملية التحليل ويرجع task_id
     */
    detectConflict: () => buildUrl(API_CONFIG.PREFIX.AI, 'detect-conflict/'),

    /** GET /api/analysis/conflict-status/?task_id={id}
     *  polling — يرجع { state, report_id? }
     */
    conflictStatus: (taskId: string) =>
      buildUrl(API_CONFIG.PREFIX.AI, `conflict-status/?task_id=${taskId}`),

    /** GET /api/analysis/conflict-result/{reportId}/
     *  يرجع النتيجة الكاملة بعد state === 'SUCCESS'
     */
    conflictResult: (reportId: string) =>
      buildUrl(API_CONFIG.PREFIX.AI, `conflict-result/${reportId}/`),
  },
} as const;
