...
const DocumentGenerationPage = () => {
  ...
  const getLatestAnalysisResult = useCallback(async (): Promise<AnalysisResult | null> => {
    // If we already have the analysis result from diagram generation, use it directly.
    // This ensures we always use the PROJECT-LEVEL analysis ID, not a random latest result.
    if (analysisResults?.id) {
      console.log('🎯 [DEBUG] Using cached analysis result:', analysisResults.id);
    ...
    try {
      ...
      if (response.ok) {
        ...
        const allResults: AnalysisResult[] = data.results || data || [];

        // Scope to results that belong to the current project's code file
        // analysisResults?.code_file_id is set during diagram generation
        const projectResults = analysisResults?.code_file_id
          ? allResults.filter(r => r.code_file_id === analysisResults.code_file_id)
          : allResults;

        if (projectResults.length > 0) {
          const latest = projectResults.sort((a, b) =>
            new Date(b.analysis_completed_at || 0).getTime() - new Date(a.analysis_completed_at || 0).getTime()
          )[0];
          console.log('🎯 [DEBUG] Project-scoped analysis result:', latest.id);
          return latest;
        }
        console.log('ℹ️ [DEBUG] No analysis results found for this project');
        return null;
      } else {
        console.error('❌ [DEBUG] Failed to fetch analysis results:', response.status);
        return null;
      }
    } catch (error) {
  ...
  }, [analysisResults, getAuthHeaders]);
  ...
  const generateCodeExplanation = async (): Promise<string | null> => {
    ...
    try {
      ...
      // URL: /api/explanation/export-legacy/?id={PROJECT_ANALYSIS_ID}&format=md&type={level}
  ...
  const generateDocument = async (action: 'display' | 'download') => {
    ...
    try {
      ...
      // URL: /api/explanation/export-legacy/?id={PROJECT_ANALYSIS_ID}&format={fmt}&type={level}
      const blob = await UnifiedApiService.exportAnalysis(latestResult.id, {
  ...
  // Generate initial class diagram (only for single-file flow where codeText is present)
  useEffect(() => {
    ...
    // Only auto-generate if codeText is available (single file flow)
    // For project/ZIP flow, diagram is already generated and shown in ClassDiagramPage
    if (state?.codeText && state.codeText.trim().length > 1 && state?.projectId) {
