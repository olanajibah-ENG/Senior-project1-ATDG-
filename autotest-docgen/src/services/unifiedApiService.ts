...
class UnifiedApiService {
   ...
   * Uses /api/upm/projects/{project_id}/folder-upload/ which correctly
   * saves content to GridFS and triggers analysis via Celery.
   */
  static async uploadFilesWithProject(data: {
    ...
    try {
      ...
      // POST to folder-upload — handles single/multiple files, ZIP, versioning, GridFS + analysis
      const response = await apiClient.post(
        `/api/upm/projects/${data.project_id}/folder-upload/`,
        formData,
        {
      ...
      console.log('✅ Files uploaded via folder-upload:', response.data);
      return response.data; // { task_id, status, message, project_id, check_status }
    } catch (error) {
  ...
  /**
   * Poll until AI service has received the uploaded files for a project.
   * The upload goes through UPM → Celery → AI (async), so we must wait
   * before triggering analysis.
   */
  static async waitForFilesInAI(
    projectId: string,
    expectedFileCount: number = 1,
    ...
    while (Date.now() < deadline) {
      ...
      try {
        // project-versions endpoint returns file_ids once AI has processed the upload
        const response = await apiClient.get(`/api/analysis/project-versions/${projectId}/`);
        const versions = response.data?.versions ?? response.data;
        if (Array.isArray(versions) && versions.length > 0) {
          const latestVersion = versions[versions.length - 1];
          const fileCount = latestVersion?.total_files ?? latestVersion?.file_ids?.length ?? 0;
          console.log(`⏳ Waiting for AI files (attempt ${attempt}): found ${fileCount}/${expectedFileCount}`);
          if (fileCount >= expectedFileCount) {
            console.log('✅ Files confirmed in AI service');
            return;
          }
        }
      } catch (e) {
        // 404 means not yet — keep waiting
        console.log(`⏳ Waiting for AI files (attempt ${attempt}): not ready yet`);
      }
    }

    throw new Error('Timed out waiting for files to reach AI service. Please try again.');
  }
