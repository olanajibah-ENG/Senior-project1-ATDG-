...
class ExtendedApiService {
  ...
  static async getDependencyGraph(projectId: string, fileNames: string[]): Promise<DependencyGraph> {
    try {
      ...
      console.warn('⚠️ dependency-graph endpoint not available, returning empty graph');
      return { nodes: [], edges: [] };
    }
  }
  ...
  static async getCrossFileContext(projectId: string, fileNames: string[]): Promise<CrossFileContext> {
    try {
      ...
      console.warn('⚠️ cross-file-context endpoint not available, returning empty context');
      return { fileContexts: [], file_relationships: [] };
    }
  }
