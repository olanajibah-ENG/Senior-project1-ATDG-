...
class TreeSitterProcessor(ILanguageProcessorStrategy):
    ...
    def _extract_generic_constraints(self, class_node) -> List[str]:
        """
        يستخرج الـ generic constraints من الـ where clause.
        يدعم C# (where T : X, new()), Java (extends X & Y), TypeScript (T extends X).
        يعمل مع أي لغة تدعمها tree-sitter بالبحث عن node types معروفة.
        """
        ...
        for node in class_node.children:
            if node.type not in CONSTRAINT_NODE_TYPES:
                if node.type in ("type_parameters", "type_parameter_list"):
                    for child in node.children:
                        if child.type in CONSTRAINT_NODE_TYPES:
                            constraints.extend(
                                TreeSitterProcessor._parse_constraint_node(child)
                            )
                continue
            constraints.extend(TreeSitterProcessor._parse_constraint_node(node))

        if not constraints:
            raw = TreeSitterProcessor._node_text(class_node) or ""
            constraints = TreeSitterProcessor._parse_constraints_from_text(raw)

        return constraints

    @staticmethod
    def _parse_constraint_node(node) -> List[str]:
        """يستخرج أسماء الـ constraints من node واحد"""
        results = []
        for child in node.children:
            if child.type in (
                ...
                if name.lower() not in ("where", "t", "extends", "implements", ":"):
                    results.append(name)
            elif child.type == "constructor_constraint" or (
                child.text and b"new()" in child.text
            ):
                results.append("new()")

        if not results:
            text = TreeSitterProcessor._node_text(node) or ""
            results = TreeSitterProcessor._parse_constraints_from_text(text)

        return results

    @staticmethod
    def _parse_constraints_from_text(text: str) -> List[str]:
        """
        Fallback: يستخرج الـ constraints من النص الخام.
        يبحث عن "where T : X, Y" أو "extends X & Y".
        """
        import re
        constraints = []
        ...
        if m:
            for part in m.group(1).split(","):
                c = part.strip().split()[0] if part.strip() else ""
                if c and c.lower() not in ("class", "struct", "unmanaged", "notnull"):
                    constraints.append(c)
            return constraints

        m = re.search(r'\bextends\s+([\w<>,\s&]+)', text, re.IGNORECASE)
        if m:
            import re as _re
            for part in _re.split(r'[&,]', m.group(1)):
                c = part.strip().split()[0] if part.strip() else ""
                if c:
                    constraints.append(c)
