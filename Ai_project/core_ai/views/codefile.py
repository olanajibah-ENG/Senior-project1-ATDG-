    ...
    def create(self, request):
        ...
        if 'uploaded_file' in validated_data and validated_data['uploaded_file']:
            ...
            try:
                ...
                # ✅ Save content to GridFS so the analysis task can retrieve it
                gridfs_id = save_to_gridfs(
                    content=file_content,
                    filename=uploaded_file.name,
                    metadata={'file_type': validated_data.get('file_type', 'unknown')},
                )
                validated_data['gridfs_id'] = gridfs_id
                logger.info(f"--- [CF-CREATE] Content saved to GridFS: {gridfs_id} ---")
            except Exception as e:
        ...
        import hashlib
        raw_content = validated_data.get('content') or validated_data.get('code_content', '')
        if not raw_content:
        ...
        # ✅ إذا لم يُحفظ في GridFS بعد (حالة JSON content بدون uploaded_file)
        if not validated_data.get('gridfs_id'):
            try:
                gridfs_id = save_to_gridfs(
                    content=raw_content,
                ...
                validated_data['gridfs_id'] = gridfs_id
                logger.info(f"--- [CF-CREATE] Content saved to GridFS (JSON path): {gridfs_id} ---")
            except Exception as e:
                logger.error(f"--- [CF-CREATE] Failed to save to GridFS: {e} ---")
        ...
        try:
            ...
            data_to_insert = code_file_instance.model_dump(by_alias=True, exclude_unset=True)
