...
export class ApiService extends BaseService {
    ...
    async createCodeFile(codeFileData: CreateCodeFileData): Promise<CodeFile> {
        // For FormData uploads, we need to use UPM artifacts endpoint
        if (codeFileData instanceof FormData) {
            const projectId = codeFileData.get('source_project_id');
            return this.post<CodeFile, FormData>(`/upm/projects/${projectId}/artifacts/`, codeFileData);
        }
    ...
    async getGeneratedFiles(): Promise<GeneratedFile[]> {
        const response = await this.get<{ files: GeneratedFile[] }>(
            API_ENDPOINTS.analysis.generatedFiles()
        );
    ...
    async getProjectCodes(projectId: string): Promise<ProjectCode[]> {
        try {
            const response = await this.get<any[]>(`/upm/projects/${projectId}/artifacts/`);

            return response.map(artifact => ({
                id: artifact.code_id || artifact.id,
                code_name: artifact.code_name || artifact.file_name || 'Unknown',
                file_name: artifact.file_name || 'Unknown',
                language: artifact.code_language || 'python',
                version: artifact.code_version || '1.0',
                created_at: artifact.upload_date || artifact.created_at,
    ...
    async getProjectDocumentation(projectId: string): Promise<ProjectDocumentation[]> {
        try {
            ...
            return response
                .filter(artifact => artifact.artifact_type === 'documentation' || artifact.file_type === 'documentation')
                .map(artifact => ({
                    id: artifact.code_id || artifact.id,
                    name: artifact.code_name || artifact.file_name || 'Unknown',
                    type: artifact.artifact_type || 'class_diagram',
                    created_at: artifact.upload_date || artifact.created_at,
                    file_path: artifact.storage_reference || artifact.file_path
                }));
        } catch (error) {
