...
import {
  ...
  AlertCircle, CheckCircle, Moon, Sun
} from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-bold" style={{ color: 'var(--ad-text-primary)', fontWeight: 800 }}>
                Admin Dashboard
              </h1>
            </div>
            <div className="flex items-center gap-3">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="ad-card ad-anim-fadeup mb-8">
          <div className="ad-tabs-nav">
            <button onClick={() => setActiveTab('overview')} className={`ad-tab ${activeTab === 'overview' ? 'active' : ''}`}>
              <BarChart3 className="w-4 h-4" /> Overview
            </button>
            <button onClick={() => setActiveTab('users')} className={`ad-tab ${activeTab === 'users' ? 'active' : ''}`}>
              <Users className="w-4 h-4" /> User Management
              {adminUsers.length > 0 && <span className="ad-tab-badge">{adminUsers.length}</span>}
            </button>
            <button onClick={() => setActiveTab('online')} className={`ad-tab ${activeTab === 'online' ? 'active' : ''}`}>
              <Activity className="w-4 h-4" /> Online Users
              {onlineUsers.length > 0 && <span className="ad-tab-badge">{onlineUsers.length}</span>}
            </button>
          </div>
        </div>
