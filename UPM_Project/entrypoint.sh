...
if [ -n "$CELERY_WORKER" ]; then
    ...
    # ✅ workers زدناها من 2 لـ 4 — كل worker يعالج طلب مستقل
    # ✅ threads=2 — كل worker يقدر يشغّل طلبين بالتوازي
    # ✅ timeout زدناه لـ 180 للعمليات الثقيلة
    # ✅ worker-connections=1000 — أقصى اتصالات متزامنة
    exec gunicorn UPM_Project.wsgi:application \
        --bind 0.0.0.0:8000 \
        --workers 4 \
        --threads 2 \
        --timeout 180 \
